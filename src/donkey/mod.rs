...
unsafe extern "C" fn donkey_game_specialairhi(agent: &mut L2CAgentBase) {
    ...
    let mut dk_upB_nopress = true;
    let mut dk_upB_press = false;
    let mut dk_upB_boostcount = 0;
    ...
    for _ in 0..7 {
        ...
        if dk_upB_boostcount < 4 {
            if ControlModule::check_button_on(agent.module_accessor, *CONTROL_PAD_BUTTON_SPECIAL) && dk_upB_nopress == true { 
